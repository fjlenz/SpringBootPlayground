...
# 1 Build docker image locally with Dockerfile: docker build --pull --rm -f "Dockerfile" -t gcr.io/projectfj/spring-boot-playground-no-db:1.0.5 "."
# 2 check if image got created: docker images
# 3 Push to Google-Container-Registry (GCR): docker push gcr.io/projectfj/spring-boot-playground-no-db:1.0.5
